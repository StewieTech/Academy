...
// A feature could be a UI example of all the different enum types for the lexical analyzer. The user would be able 
/*
...
OPERATOR
performs math operations on values and operations on variables // again will probably need lots of exceptions here
+ (addition)
- (subtraction)
* (multiplication)
/ (division)
...
WHITESPACE
// not only spaces, tabs, newlines but also non-visible characters. I am not sure what non visible characters are
(space)
\t (tab)
\n (newline)
...
COMMENT
can't be executed and meant for documentation. // need to handle single and double line examples
// single comment
/* multi comment */
